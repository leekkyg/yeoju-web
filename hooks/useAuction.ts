...
export function useAuction(auctionId: string) {
  ...
  const fetchAuction = useCallback(async () => {
    ...
    try {
      ...
      supabase.from('auctions').update({ view_count: (auctionData.view_count || 0) + 1 }).eq('id', auctionId);
...
export function useCreateAuction() {
  ...
  const createAuction = async (input: CreateAuctionInput): Promise<Auction | null> => {
    ...
    try {
      ...
      if (createError) {
        console.error('Supabase 에러 코드:', createError.code);
        console.error('Supabase 에러 메시지:', createError.message);
        console.error('Supabase 에러 상세:', createError.details);
        console.error('Supabase 에러 힌트:', createError.hint);
        throw createError;
      }
